...
function filterPirates (pirates) {
  ...
  for (var k = 0; k < pirates.length; k++){
    var check = false
    for (var l = 0; l < pirates[k].name.length; l++){
      if (pirates[k].name[l] === '.'){
        check = true
      }
    }
  ...
  for (var a = 0; a < tmp.length; a++){
    var obj = {}
    if (tmp[a].location === 'Jakarta'){
      obj.name = tmp[a].name
      obj.ID = str + '-' + jakarta
      result.push(obj)
    }
    if (tmp[a].location === 'Bogor'){
      obj.name = tmp[a].name
      obj.ID = str + '-' + bogor
      result.push(obj)
    }
    if (tmp[a].location === 'Depok'){
      obj.name = tmp[a].name
      obj.ID = str + '-' + depok
      result.push(obj)
    }
    if (tmp[a].location === 'Bekasi'){
      obj.name = tmp[a].name
      obj.ID = str + '-' + bekasi
      result.push(obj)
    }
